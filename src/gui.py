...
class GUI:
    ...
    def create_gui(self):
            ...
            [frameelevation,
             PyGUI.Column(
                 [
                     ...
                     [PyGUI.Checkbox("FullScreen", default=False, key="fullscreen", enable_events=True)],
        ...
        col0 = [
            ...
            [PyGUI.Text("Modified by Koelooptiemanna")]
        ]
        ...
        col1 = [
            ...
            [PyGUI.Button("Enter Current MSN into Current Station aircraft", key="entercur")],
        ]
    ...
    def set_sequence_station_selector(self, mode):
        ...
        if mode == "sequence":
            ...
            self.window.Element("sequence_text").Update(value="N.A.:")
            self.window.Element("sequence").Update(
                values=(1, 8, 7, 3, 2), value=1, disabled=True)
...
#                values=(8, 7, 3, 2), value=8, disabled=False)
    ...
    def capture_map_coords(self, x_start=0, y_start=0, x_end=100, y_end=25):
        self.logger.debug("Attempting to capture map coords")
        image = ImageGrab.grab((x_start, y_start, x_end, y_end))
    ...
    def input_parsed_coords(self):
        try:
            wh=FindWindow(None, "Digital Combat Simulator")
            wr=GetWindowRect(wh)
            ww=wr[2]-wr[0] #window width
            wh=wr[3]-wr[1] #window height
            if self.values['fullscreen']:
                coord_tl=((ww/100)*5.75, (wh/100)*0.86) #top left percentage to pixel
                coord_br=((ww/100)*21.41,(wh/100)*2.67) #bottom right percentage to pixel
            else:
                coord_tl=(wr[0]+(ww/100)*6.22, wr[1]+(wh/100)*2.07) #top left percentage to pixel
                coord_br=(wr[0]+(ww/100)*22.49, wr[1]+(wh/100)*4.04) #bottom right percentage to pixel
            captured_coords = self.capture_map_coords(coord_tl[0],coord_tl[1],coord_br[0],coord_br[1])
    ...
    def enter_cur_coords_to_aircraft(self,i):
        self.window.Element('entercur').Update(disabled=True)
        self.editor.enter_current_msn(i)
        self.window.Element('entercur').Update(disabled=False)
    ...
    def run(self):
        while True:
            ...
            if event is None or event == 'Exit':
                ...
                for wp in self.profile.waypoints:
                    if str(wp) == valuestr:
                        self.enter_cur_coords_to_aircraft(wp)
                        break
